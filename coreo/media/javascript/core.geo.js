...
(function($, ns) {
	...
	$.extend(KmlDomGeoDataFeature.prototype, GeoDataFeature.prototype, {
		...
		walkChildren: function(callback) {
			...
			/*
			$(this.node).children(KML_FEATURE_ELEMENTS.join(",")).each(function(index, element) {
				// assign an ID to this element if it doesn't have one
				var id = $(element).attr(self.nsPrefix + ":id");
				if (!id) {
					id = GeoDataStore.generateId();
	...
	var parseKmlFeatureChildElements = function(node, callback) {
		core.util.parseChildElementsByName(node, FEATURE_ELEMENT_NAMES, callback);
	};
	
	var KmlDomGeoData = function(kmlDomNode, parent) {
		GeoData.call(this);
		this._kmlDomNode = kmlDomNode;
	...
	$.extend(KmlDomGeoData.prototype, GeoData.prototype, {
		getKmlString: function() {
			if (!this._kmlString) {
				this._kmlString = core.util.getXmlString(this._kmlDomNode);
			}
			return this._kmlString;
		},
		getKmlDomNode: function() {
			return this._kmlDomNode;
		},
		getParent: function() {
			return this._parent;
		},
		getChildren: function() {
			if (!this._children) {
				this._children = getKmlFeatureChildElements(this.getKmlDomNode());
			}
	...
	$.extend(KmlStringGeoData.prototype, GeoData.prototype, {
		...
		getKmlDomNode: function() {
			if (!this._kmlDomNode) {
				this._kmlDomNode = core.util.createXmlDoc(this._kmlString);
			}
			return this._kmlDomNode;
		}
	});
	*/
	...
	var GeoDataFactory = {
		...
		fromKmlDom: function(kmlDom) {
			if (kmlDom.documentElement) {
				kmlDom = kmlDom.documentElement;
			}
	 ...
	 * GeoDataStore
	 * 
	 * The database of all GeoData objects.
	 */
	/*
	var GeoDataStore = {
		
		save: function(geoData) {
			datastore[geoData.id] = geoData;
		},
